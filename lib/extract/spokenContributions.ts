...
async function parseAndStoreMeeting(args: {
  ...
  for (const r of rows.items) {
    ...
    const upsertRow = {
      ...
      context_en: r.contextEn ?? undefined,
      context_cy: r.contextCy ?? undefined,
      snippet_en: content.snippetEn ?? content.snippetCy ?? "",
      snippet_cy: content.snippetCy ?? undefined,
      full_text_en: content.fullTextEn ?? undefined,
      full_text_cy: content.fullTextCy ?? undefined,
    ...
    localDb.spokenContributions.set(`${args.meetingId}:${upsertRow.contribution_id}`, upsertRow);
  }
  ...
  if (toUpsert.length > 0 && isConvexConfigured()) {
    try {
      ...
      if (client) {
        await client.mutation(api.contributions.upsertMany, { contributions: toUpsert });
      }
    } catch {
      // Ignore Convex mutation error
    }
  }
...
function extractTextContent(args: {
  verbatimHtml?: string;
  translatedHtml?: string;
  contributionLanguage?: string;
}) {
