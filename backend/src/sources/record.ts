...
export async function listRecentPlenaryExports(db: Db, limit = 10) {
  const res = await cachedFetchText(db, { url: EXPORT_URL, source: SOURCE, ttlSeconds: env.cacheTtlSeconds });
  if (res.status < 200 || res.status >= 300) throw new Error(`XMLExport fetch failed (${res.status})`);
  ...
  for (const row of rows) {
    ...
    for (const a of links) {
      ...
      switch (xmlDownloadType) {
        case "BilingualTranscript":
          item.transcriptBilingualUrl = href;
          break;
        case "WelshTranscript":
          item.transcriptWelshUrl = href;
          break;
        case "EnglishTranscript":
          item.transcriptEnglishUrl = href;
          break;
        case "Votes":
          item.votesBilingualUrl = href;
          break;
        default:
          break;
      }
