...
function isSeneddTvUrl(url: string) {
  try { return new URL(url).hostname.toLowerCase().includes("senedd.tv"); } catch { return false; }
}
...
function parseVoteFromItem(it: api.ParticipationItem, lang: "en" | "cy"): VoteParsed {
  if (it.vote) {
    const overall = (lang === "cy" ? it.vote.overallCy : it.vote.overallEn) ?? it.vote.overallEn ?? it.vote.overallCy ?? null;
  ...
  return {
    ...
    totals: totalsMatch ? { for: parseInt(totalsMatch[1], 10), against: parseInt(totalsMatch[2], 10), abstain: parseInt(totalsMatch[3], 10) } : null,
  };
}
...
function VoteResultCard({ parsed, t }: { parsed: VoteParsed; t: (k: string) => string }) {
  ...
  const memberClass = parsed.memberResult === "for" ? "vote-verdict--for" : parsed.memberResult === "against" ? "vote-verdict--against" : parsed.memberResult === "abstain" ? "vote-verdict--abstain" : "vote-verdict--neutral";
  const memberLabel = parsed.memberResult === "for" ? t("vote_for") : parsed.memberResult === "against" ? t("vote_against") : parsed.memberResult === "abstain" ? t("vote_abstain") : parsed.memberResult === "did_not_vote" ? t("vote_did_not_vote") : t("vote_unknown");
      ...
      {parsed.totals && (
        <div className="vote-result-card__tally">
          ...
          <div className="vote-tally-labels">
            <span className="vote-tally-labels__for"><span className="vote-tally-dot vote-tally-dot--for" />{t("vote_for")} <strong>{parsed.totals.for}</strong></span>
            {parsed.totals.abstain > 0 && <span className="vote-tally-labels__abstain"><span className="vote-tally-dot vote-tally-dot--abstain" />{t("vote_abstain")} <strong>{parsed.totals.abstain}</strong></span>}
            <span className="vote-tally-labels__against"><span className="vote-tally-dot vote-tally-dot--against" />{t("vote_against")} <strong>{parsed.totals.against}</strong></span>
          </div>
        </div>
      )}
    </div>
...
function TabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button className={`member-tab ${active ? "member-tab--active" : ""}`} onClick={onClick} type="button">{children}</button>;
}
...
export default function MemberPage() {
  ...
  const openSpeechReader = async (it: api.ParticipationItem) => {
    ...
    if (!match) { toast.show(t("full_text_unavailable"), "error"); return; }
    ...
    try {
      const detail = await api.getSpokenContributionDetail({ meetingId: meetingIdFromId, contributionId, memberId: id });
  ...
  useEffect(() => {
    if (!filteredTopics.length) { setSelectedTopicKey(null); return; }
  ...
  const filteredVotes = useMemo(() => {
    ...
    if (voteFilter === "cast") return items.filter((v) => { const r = parseVoteFromItem(v, lang).memberResult; return r === "for" || r === "against" || r === "abstain"; });
  ...
  return (
    <>
        ...
        <div className="container">
          <div className="member-hero__back" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button
              ...
              onClick={() => navigator.clipboard.writeText(window.location.href).then(() => toast.show(t("copied"), "ok"))}
              title={t("copy_link")} type="button"
            >
      ...
      <div className="container" style={{ paddingTop: 18, paddingBottom: 44 }}>
        {!loadingData && (
          <div className="member-tabs" role="tablist" aria-label={t("member_tabs")}>
            <TabButton active={tab === "overview"} onClick={() => setActiveTab("overview")}>{t("tab_overview")}</TabButton>
            <TabButton active={tab === "contributions"} onClick={() => setActiveTab("contributions")}>
              {t("tab_contributions")} {realContributions.length ? <span className="member-tab__count">{realContributions.length}</span> : null}
            </TabButton>
            <TabButton active={tab === "votes"} onClick={() => setActiveTab("votes")}>
              {t("tab_votes")} {realVotes.length ? <span className="member-tab__count">{realVotes.length}</span> : null}
            </TabButton>
            <TabButton active={tab === "sources"} onClick={() => setActiveTab("sources")}>{t("tab_sources")}</TabButton>
          </div>
        )}
              ...
              <section className="card">
                <div className="card__header">
                  ...
                  <div className="row row--8">
                    ...
                    <button
                      className="btn btn--ghost btn--sm" disabled={refreshing}
                      onClick={async () => {
                        ...
                        try {
                          ...
                          setRefreshError(msg); toast.show(msg, "error");
            ...
            <section className="card">
              <div className="card__header">
                ...
                {participation?.real?.partial
                  ...
                  : <span className="badge badge--ok">{t("status_available")}</span>}
              </div>
              ...
              {participation?.real?.dataNotes?.length ? (
                <div className="stack stack--4" style={{ marginBottom: 4 }}>
                  {participation.real.dataNotes.map(noteKeyToTranslationKey).filter(Boolean).map((k) => (
              ...
              <div className="row row--8" style={{ justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button className="btn btn--ghost btn--sm" type="button" onClick={() => setActiveTab("contributions")}>{t("view_topics")}</button>
              </div>
            ...
            <section className="overview-panels">
              <div className="card">
                <div className="card__header">
                  ...
                  <button className="btn btn--ghost btn--sm" type="button" onClick={() => setActiveTab("contributions")}>{t("view_all_contributions")}</button>
                </div>
                {realContributions.length ? (
                  <div className="mini-list">
                    {realContributions.slice().sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).slice(0, 3).map((it) => {
                      const snippet = (lang === "cy" ? it.snippetCy : it.snippetEn) ?? it.snippetEn;
                      return (
                        <div className="mini-item" key={it.id}>
                          <div className="mini-item__meta">
                            ...
                            {confidenceBadge(it.confidence, it.confidence === "high" ? t("confidence_high") : it.confidence === "medium" ? t("confidence_medium") : t("confidence_low"))}
                          </div>
                          ...
                          <div className="mini-item__actions">
                            <button className="btn btn--ghost btn--sm" type="button" onClick={() => void openSpeechReader(it)}>{t("read_full")}</button>
                          </div>
                        </div>
                      );
                    })}
              ...
              <div className="card">
                <div className="card__header">
                  ...
                  <button className="btn btn--ghost btn--sm" type="button" onClick={() => setActiveTab("votes")}>{t("view_all_votes")}</button>
                </div>
                {realVotes.length ? (
                  <div className="mini-list">
                    {realVotes.slice().sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).slice(0, 3).map((it) => {
                      const parsedVote = parseVoteFromItem(it, lang);
                      const badgeClass = parsedVote.memberResult === "for" ? "badge--ok" : parsedVote.memberResult === "against" ? "badge--danger" : parsedVote.memberResult === "abstain" ? "badge--warn" : "badge--neutral";
                      const label = parsedVote.memberResult === "for" ? t("vote_for") : parsedVote.memberResult === "against" ? t("vote_against") : parsedVote.memberResult === "abstain" ? t("vote_abstain") : parsedVote.memberResult === "did_not_vote" ? t("vote_did_not_vote") : t("vote_unknown");
                      return (
                        <div className="mini-item" key={it.id}>
                          <div className="mini-item__meta">
                            <span className="mini-item__date">{formatDate(it.occurredAt)}</span>
              ...
              <div className="card">
                <div className="card__header">
                  ...
                  <button className="btn btn--ghost btn--sm" type="button" onClick={() => setActiveTab("contributions")}>{t("view_all_committees")}</button>
                </div>
                {committeeMeetings.length ? (
                  <div className="mini-list">
                    {committeeMeetings.slice().sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).slice(0, 3).map((it) => {
                      const snippet = (lang === "cy" ? it.snippetCy : it.snippetEn) ?? it.snippetEn;
                      return (
                        <div className="mini-item" key={it.id}>
                          <div className="mini-item__meta">
              ...
              {realContributions.length === 0 ? (
                ...
                <div className="topic-layout">
                  <div className="topic-sidebar">
                    <input
                      className="input topic-search" value={topicQuery}
                      onChange={(e) => setTopicQuery(e.target.value)}
                      placeholder={t("topic_search_placeholder")} aria-label={t("topic_search_placeholder")}
                    />
                    <div className="topic-list" role="list">
                      {filteredTopics.map((x) => {
                        ...
                        return (
                          <button type="button" key={x.key} className={`topic-list__item ${active ? "topic-list__item--active" : ""}`} onClick={() => setSelectedTopicKey(x.key)}>
                    ...
                    {selectedTopic ? (
                      <>
                            ...
                            return (
                              <div className="topic-entry" key={s.id}>
                                <div className="topic-entry__meta">
                                  ...
                                  {confidenceBadge(s.confidence, s.confidence === "high" ? t("confidence_high") : s.confidence === "medium" ? t("confidence_medium") : t("confidence_low"))}
                                </div>
                                ...
                                <div className="topic-entry__actions">
                                  <button className="btn btn--ghost btn--sm" type="button" onClick={() => void openSpeechReader(s)}>{t("read_full")}</button>
                                  {showRecordLink && recordUrl && (
                                    <a className="btn btn--ghost btn--sm" href={recordUrl} target="_blank" rel="noreferrer">{t("view_record_page")} <ExternalIcon /></a>
                                  )}
                                  {showSourceLink && (
                        ...
                        {selectedTopic.items.length > 30 && (
                          <div className="text-xs text-muted" style={{ marginTop: 10 }}>{t("showing_latest_n").replace("{n}", "30")}</div>
                        )}
                      </>
              ...
              {committeeMeetings.length === 0 ? (
                ...
                <div className="data-table-wrap">
                  <table className="data-table">
                    <thead><tr><th>{t("table_date")}</th><th>{t("table_item")}</th><th>{t("table_status")}</th><th className="data-table__actions">{t("table_actions")}</th></tr></thead>
                    <tbody>
                      {committeeMeetings.slice().sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).slice(0, 14).map((it) => {
                        const snippet = (lang === "cy" ? it.snippetCy : it.snippetEn) ?? it.snippetEn;
                        return (
                          <tr key={it.id}>
                            <td className="data-table__muted">{formatDate(it.occurredAt)}</td>
                            <td><div className="data-table__title">{it.title}</div>{snippet ? <div className="data-table__sub">{snippet}</div> : null}</td>
                            <td>{confidenceBadge(it.confidence, t("confidence_high"))}</td>
                            <td className="data-table__actions"><a className="btn btn--ghost btn--sm" href={it.sourceUrl} target="_blank" rel="noreferrer">{t("source_label")} <ExternalIcon /></a></td>
                          </tr>
                        );
                      })}
                    </tbody>
              ...
              {realVotes.length === 0 ? (
                ...
                <>
                  <div className="vote-filter" role="group" aria-label={t("vote_filter_label")}>
                    {(["cast","all","for","against","abstain","did_not_vote"] as VoteFilter[]).map((f) => (
                      <button key={f} type="button" className={`vote-filter__btn ${voteFilter === f ? "vote-filter__btn--active" : ""}`} onClick={() => setVoteFilter(f)}>
                        {f === "cast" ? t("vote_filter_cast") : f === "all" ? t("vote_filter_all") : f === "for" ? t("vote_for") : f === "against" ? t("vote_against") : f === "abstain" ? t("vote_abstain") : t("vote_did_not_vote")}
                        <span className="vote-filter__count">{voteCounts[f]}</span>
                      </button>
                    ))}
                  </div>
                  <div className="stack stack--8" style={{ marginTop: 12 }}>
                        ...
                        <details className="vote-item" key={it.id}>
                          <summary className="vote-item__summary">
                            ...
                            <div className="vote-item__right">
                              <span className={`badge ${parsedVote.memberResult === "for" ? "badge--ok" : parsedVote.memberResult === "against" ? "badge--danger" : parsedVote.memberResult === "abstain" ? "badge--warn" : "badge--neutral"}`}>
                                {parsedVote.memberResult === "for" ? t("vote_for") : parsedVote.memberResult === "against" ? t("vote_against") : parsedVote.memberResult === "abstain" ? t("vote_abstain") : parsedVote.memberResult === "did_not_vote" ? t("vote_did_not_vote") : t("vote_unknown")}
                              </span>
                            </div>
                          </summary>
                          <div className="vote-item__body">
                            ...
                            <div className="row row--8" style={{ marginTop: 10, flexWrap: "wrap" }}>
                              ...
                              {showSourceLink && (
                                <a className="btn btn--ghost btn--sm" href={it.sourceUrl} target="_blank" rel="noreferrer">{t("source_label")} <ExternalIcon /></a>
                              )}
                              {confidenceBadge(it.confidence, it.confidence === "high" ? t("confidence_high") : it.confidence === "medium" ? t("confidence_medium") : t("confidence_low"))}
                            </div>
                          </div>
                    ...
                    {filteredVotes.length > 40 && <div className="text-xs text-muted">{t("showing_latest_n").replace("{n}", "40")}</div>}
                  </div>
                </>
              )}
            </section>
            ...
            <section className="card">
              ...
              {exports?.items?.length ? (
                <div className="stack stack--8">
                  {exports.items.map((it, idx) => {
                    const anyUrl = it.transcriptBilingualUrl ?? it.transcriptEnglishUrl ?? it.transcriptWelshUrl ?? it.votesBilingualUrl;
                    ...
                    return (
                      <div className="card source-card" key={`${it.title}-${idx}`} style={{ padding: "16px" }}>
                        ...
                        {it.dateText && <span className="text-xs text-muted" style={{ marginBottom: 10, display: "block" }}>{it.dateText}</span>}
                        {meetingId ? (
                          <a className="btn btn--ghost btn--sm" href={recordPageUrl(meetingId)} target="_blank" rel="noreferrer" style={{ display: "inline-flex" }}>
              ...
              {exports && (
                <div className="text-sm text-muted" style={{ marginTop: 10 }}>
                  {t("source_label")}: <a href={exports.sourceUrl} target="_blank" rel="noreferrer">{new URL(exports.sourceUrl).host}</a> · {exports.fromCache ? t("cache_cached") : t("cache_live")}
                </div>
              )}
            </section>
          </div>
      ...
      {readerOpen && (
        <div className="modal-backdrop" role="dialog" aria-modal="true" onMouseDown={(e) => { if (e.target === e.currentTarget) setReaderOpen(false); }}>
          <div className="modal">
            <div className="modal__header">
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <div className="modal__title">{t("read_full")}</div>
                {readerDetail ? (
                  <div className="text-xs text-muted">{readerDetail.speakerName} · {new Date(readerDetail.occurredAt).toLocaleString()}</div>
                ) : null}
              </div>
              <button className="btn btn--ghost btn--sm" onClick={() => setReaderOpen(false)} type="button">{t("close")}</button>
            </div>
            <div className="modal__body">
              <div className="row row--8" style={{ justifyContent: "space-between", marginBottom: 12 }}>
                <div className="row row--8">
                  ...
                  <select className="nav__lang-select" value={readerLang} onChange={(e) => setReaderLang(e.target.value === "cy" ? "cy" : "en")} aria-label={t("language_label")}>
                ...
                {readerDetail ? (
                  <div className="row row--8">
                    <a className="link-pill" href={readerDetail.recordPageUrl} target="_blank" rel="noreferrer">{t("view_record_page")} <ExternalIcon /></a>
                    <a className="link-pill" href={readerDetail.sourceUrl} target="_blank" rel="noreferrer">{isSeneddTvUrl(readerDetail.sourceUrl) ? t("watch_video") : t("official_source")} <ExternalIcon /></a>
                  </div>
                ) : null}
              </div>
              {readerLoading ? (
                ...
                <>
                  {(readerLang === "cy" ? readerDetail.fullTextCy : readerDetail.fullTextEn) ? (
                    <div className="reader-text">{(readerLang === "cy" ? readerDetail.fullTextCy : readerDetail.fullTextEn) ?? ""}</div>
                  ) : (readerLang === "cy" ? readerDetail.fullTextEn : readerDetail.fullTextCy) ? (
                    <>
                      <div className="alert alert--info text-sm" style={{ marginBottom: 10 }}>{t("full_text_fallback_other_language")}</div>
                      <div className="reader-text">{(readerLang === "cy" ? readerDetail.fullTextEn : readerDetail.fullTextCy) ?? ""}</div>
                    </>
                  ) : (
