...
// Simple keyword matching (no AI). Includes a small set of Welsh/English stems.
const RULES: TopicRule[] = [
  ...
  { topic: "Housing", keywords: ["housing", "home", "homes", "rent", "rental", "tenant", "landlord", "homeless", "planning", "tai", "rhent", "tenant", "digartref", "cynllunio"] },
  { topic: "Transport", keywords: ["transport", "bus", "rail", "train", "tram", "road", "roads", "traffic", "cycling", "metro", "trafnidiaeth", "bws", "rheil", "tren", "ffordd", "traffig", "beicio", "metro"] },
  ...
  { topic: "Environment", keywords: ["environment", "climate", "net zero", "carbon", "biodiversity", "flood", "pollution", "renewable", "green", "amgylchedd", "hinsawdd", "sero net", "carbon", "bioamrywiaeth", "llifogydd", "llygredd", "adnewyddadwy", "gwyrdd"] },
  { topic: "Welsh Language", keywords: ["welsh language", "cymraeg", "language", "bilingual", "welsh-medium", "iaith", "dwyieith", "ysgolion cyfrwng cymraeg"] },
...
export function topicBreakdownFromItems(items: Array<{ title?: string; snippetEn?: string; snippetCy?: string; contextEn?: string; contextCy?: string }>) {
  const counts = new Map<Topic, number>();
  for (const it of items) {
    const text = [it.title, it.contextEn, it.contextCy, it.snippetEn, it.snippetCy].filter(Boolean).join(" ");
    const topic = primaryTopicFromText(text);
